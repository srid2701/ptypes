...
class Spectra(object):
    def __init__(self, freqs, tsamp, data, DM=0.0, startT=0.0, endT=1.0):

        """"""

        self.data = data.astype("float")

        [self.numchans, self.numspecs] = self.data.shape
    ...
    def shiftchans(self, bins, padval=0):
        ...
        for cindx in cindxs:
            ...
            channel[:] = rotate(channel, bins[cindx])
    ...
    def subband(self, nsub, padval=0, subDM=None):
        ...
        subLOFREQs = self.freqs[((1 + NSUBs) * subCHANs - 1)]
        ...
        SUMs = [np.sum(SPLIT, axis=0) for SPLIT in SPLITs]

        self.data = np.array(SUMs)
        self.freqs = subCFREQs
        self.numchans = nsub

    def meanscl(self, indep=False):
    ...
    def masked(self, mask, value=0, method="mid80"):
        ...
        for cindx in cindxs:
            ...
            if method is "mean":
                ...
                num = int(np.round(0.1 * self.numspecs))
            ...
            if np.all(mask[cindx]):
                self.data[cindx] = np.ones_like(self.data[cindx]) * (
                    values[:, np.newaxis][cindx]
                )

            return self

    def dedisperse(self, DM=0, PAD=0):
    ...
    def smooth(self, width=1, padval=0, padmethod=None):

        """"""

        if width > 1:

            kernel = np.ones(width, dtpye="float32") / np.sqrt(width)
            ...
            for cindx in cindxs:
                ...
                if padmethod is not None:
                    ...
                    if padmethod is "wrap":
                        ...
                        tosmooth = np.concatenate(
                            [channel[-width:], channel, channel[:width]]
                        )

                    elif padmethod is "mean":

                        tosmooth = np.ones(self.numspecs + width * 2) * np.mean(channel)

                    elif padmethod is "median":

                        tosmooth = np.ones(self.numspecs + width * 2) * np.median(
                            channel
                        )

                else:

                    tosmooth = np.ones(self.numspecs + width * 2) / padval
    ...
    def trim(self, bins=0):
        ...
        if bins is 0:
            ...
            self.data = self.data[:, bins:]
            self.numspecs = self.numspecs - bins
            self.startT = self.startT + bins * self.tsamp

    def downsamp(self, factor=1, trime=True):
        ...
        if not trim:
            if not self.numspecs % factor:
                ...
                ERRMSG = (
                    "Downsampling factor not a factor"
        ...
        summations = [np.sum(subint, axis=1) for subint in subints]
